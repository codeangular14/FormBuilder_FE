...
export class ApiHelper {
  ...
  multiGet<T>(endpoints: Array<string>): Observable<T[]> {
    const apiArray = endpoints.map((endpoint) => {
      ...
      return this.http.get<T>(url, {
        headers: this.commonService.getTokenHeader(),
      });
    });
  ...
  getNoToken<T>(endpoint: string): Observable<T> {
    const url = `${environment.APIUrl}${endpoint}`;
    return this.http.get<T>(url, {
      headers: this.commonService.getHeaderContentTypeOnly(),
    });
  }
  ...
  postUpload<T>(endpoint: string, payload: any): Observable<T> {
    //TODO
    const url = `${environment.APIUrl}${endpoint}`;
    return this.http.post<T>(url, payload, {
      headers: this.commonService.getTokenHeaderMultiPart(),
    });
  }
  postUploadNoToken<T>(endpoint: string, payload: any): Observable<T> {
    //TODO
    const url = `${environment.APIUrl}${endpoint}`;
    return this.http.post<T>(url, payload);
  }
